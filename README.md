...
Период «N дней» считается по **дате последнего сообщения** в чате. Дата отклика на вакансию не учитывается. Поле `lastActivityTime` из Chatik для окна не используется: оно сдвигается вперёд и без новых сообщений, в том числе когда чат читает сам дашборд, поэтому по нему любое окно давало почти один и тот же список.

В сводке отчёта видно, чем этот отбор проверить: по какому полю резали, сколько чатов просмотрено в списке и какие даты сообщений попали в выборку.
...
- Список чатов обходится курсором Chatik (`nextFrom` → `from`). Параметр `page` у актуального API не двигает выдачу. Окно применяется на нашей стороне. Обход прекращается после пяти порций подряд, где в окно не попал ни один чат.
